...
The panel will be served over plain {click.style('HTTP', bold=True)} on {click.style(f'{UVICORN_HOST}:{UVICORN_PORT}', bold=True)} — traffic (including your admin login) is not encrypted.

If this host is reachable from an untrusted network, put it behind a TLS-terminating reverse proxy (Nginx/Caddy) or restrict access via firewall/VPN.
            """)
